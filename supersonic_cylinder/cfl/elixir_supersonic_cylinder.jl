...
cfl_number = 0.11 # crashes with cfl_number = 0.15
semi = TrixiLW.SemidiscretizationHyperbolic(mesh, TrixiLW.get_time_discretization(solver),
...
lift_computation = TrixiLW.AnalysisSurfaceIntegral(semi -> semi.boundary_conditions.boundary_indices[4],
                                           TrixiLW.lift_force)
drag_computation = TrixiLW.AnalysisSurfaceIntegral(semi -> semi.boundary_conditions.boundary_indices[4],
                                           TrixiLW.drag_force)
...
# AnalysisCallback MUST be before AMRCallback so that it is called first
callbacks = ( summary_callback, alive_callback, save_solution,
             analysis_callback,  amr_callback
	      )
