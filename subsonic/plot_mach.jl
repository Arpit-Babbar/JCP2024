...
plt.figure()
plt.scatter(lower[:,1], lower[:,2], label = "\$ y > 0 \$", marker = "s", facecolors = "none", edgecolors = "red")
plt.scatter(upper[:,1], upper[:,2], s = 70, label = "\$ y < 0 \$", marker = "o", facecolors="none", edgecolors="g")
plt.xlim(max(minimum(lower[:,1]), minimum(upper[:,1]))-0.1, maximum(lower[:,1])+0.1)
plt.legend()
