...
volume_integral = TrixiLW.VolumeIntegralFRShockCapturing(
   ...
   #   reconstruction = SSFR.MUSCLHancockReconstruction()
   # reconstruction=SSFR.MUSCLReconstruction()
)
...
cfl_number = 0.14 # Crashes for cfl_number = 0.14 without AMR
cfl_number = 0.14 # Crashes for cfl_number = 0.15
semi = SemidiscretizationHyperbolic(mesh, TrixiLW.get_time_discretization(solver),
...
stage_limiter! = MyPositivityPreservingLimiterZhangShu(thresholds=(5.0e-7, 1.0e-6),
