...
boundary_conditions = Dict(
		       :Wall=>TrixiLW.slip_wall_approximate,
		       :Inflow=>boundary_condition_supersonic_inflow,
                       :OutflowM => boundary_condition_subsonic_constant,
                       :OutflowP => boundary_condition_subsonic_constant
                       )


mesh_file = "grid0.inp"
boundary_symbols = [:Wall, :Inflow, :OutflowM, :OutflowP]
# mesh = P4estMesh{2}(mesh_file, initial_refinement_level=0, boundary_symbols)
mesh = P4estMesh{2}(mesh_file, polydeg = 5, boundary_symbols = boundary_symbols)
...
save_solution = SaveSolutionCallback(interval=40000,
                                     save_initial_solution=true,
                                     save_final_solution=true,
